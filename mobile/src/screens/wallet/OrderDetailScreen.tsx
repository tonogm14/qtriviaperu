...
function buildMerchReceiptHtml(o: MerchGroup): string {
...
<div class="footer">
  QTrivia Perú · Lima, Perú<br/>
  ${formatDateShort(o.createdAt)}<br/>
  <em>Este documento es un comprobante de su compra.</em>
</div>
</body>
</html>`;
}

function buildLifeReceiptHtml(o: LifeOrder): string {
  const date = formatDate(o.createdAt);
  const shortId = fmtOrderNum(o.orderNumber);
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  body { font-family:'Helvetica Neue',Arial,sans-serif; background:#fff; color:#1a1a2e; padding:32px; max-width:560px; margin:auto; }
  .logo { font-size:28px; font-weight:900; letter-spacing:-1px; color:#7C3AED; }
  .logo span { color:#FACC15; }
  .subtitle { font-size:11px; color:#888; letter-spacing:2px; text-transform:uppercase; margin-top:2px; }
  hr.thick { border:none; border-top:2px solid #EC4899; margin:18px 0; }
  hr.thin  { border:none; border-top:1px solid #eee; margin:16px 0; }
  .row { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:7px; }
  .label { font-size:11px; color:#888; }
  .value { font-size:13px; font-weight:700; text-align:right; }
  .badge { display:inline-block; padding:3px 10px; border-radius:999px; font-size:11px; font-weight:800; background:#EC489922; color:#EC4899; border:1px solid #EC489955; }
  h3 { font-size:12px; font-weight:800; text-transform:uppercase; letter-spacing:1.5px; color:#555; margin-bottom:10px; }
  .big-emoji { font-size:40px; }
  .pack-name { font-size:18px; font-weight:900; margin-top:4px; }
  .total-row { display:flex; justify-content:space-between; align-items:center; margin-top:12px; }
  .total-label { font-size:13px; color:#888; text-transform:uppercase; letter-spacing:1px; }
  .total-amount { font-size:28px; font-weight:900; color:#EC4899; }
  .footer { margin-top:28px; text-align:center; font-size:11px; color:#aaa; line-height:18px; }
  .receipt-num { font-family:monospace; font-size:12px; color:#888; }
</style>
</head>
...
<div class="row"><span class="label">N° Comprobante</span><span class="receipt-num">VIDA-${shortId}</span></div>
<div class="row"><span class="label">Fecha</span><span class="value" style="font-weight:400;font-size:12px;">${date}</span></div>
<div class="row"><span class="label">Método de pago</span><span class="value">${o.method.toUpperCase()}</span></div>
<div class="row"><span class="label">Estado</span><span class="badge">✅ Acreditado</span></div>

<hr class="thin"/>
<h3>Detalle</h3>
<div class="big-emoji">❤️</div>
<div class="pack-name">${o.packLabel}</div>
<br/>
<div class="row"><span class="label">Vidas por pack</span><span class="value">${o.lives} vida${o.lives !== 1 ? 's' : ''}</span></div>
<div class="row"><span class="label">Cantidad</span><span class="value">${o.quantity} pack${o.quantity !== 1 ? 's' : ''}</span></div>
<div class="row"><span class="label">Total vidas acreditadas</span><span class="value" style="color:#EC4899;">${totalLives} ❤️</span></div>

<hr class="thin"/>
<div class="total-row">
  <span class="total-label">Total pagado</span>
  <span class="total-amount">S/ ${o.price.toFixed(2)}</span>
</div>
...
function buildVipReceiptHtml(o: VipEntry): string {
  const date = formatDate(o.joinedAt);
  const gameDate = formatDate(o.game.scheduledAt);
  const shortId = fmtOrderNum(o.orderNumber);
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .logo span { color:#FACC15; }
  .subtitle { font-size:11px; color:#888; letter-spacing:2px; text-transform:uppercase; margin-top:2px; }
  hr.thick { border:none; border-top:2px solid #FACC15; margin:18px 0; }
  hr.thin  { border:none; border-top:1px solid #eee; margin:16px 0; }
  .row { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:7px; }
  .label { font-size:11px; color:#888; }
  .value { font-size:13px; font-weight:700; text-align:right; }
  .badge { display:inline-block; padding:3px 10px; border-radius:999px; font-size:11px; font-weight:800; background:#FACC1522; color:#B45309; border:1px solid #FACC1555; }
  h3 { font-size:12px; font-weight:800; text-transform:uppercase; letter-spacing:1.5px; color:#555; margin-bottom:10px; }
  .big-emoji { font-size:40px; }
  .game-title { font-size:20px; font-weight:900; margin-top:6px; color:#1a1a2e; }
  .total-row { display:flex; justify-content:space-between; align-items:center; margin-top:12px; }
  .total-label { font-size:13px; color:#888; text-transform:uppercase; letter-spacing:1px; }
  .total-amount { font-size:28px; font-weight:900; color:#FACC15; }
  .footer { margin-top:28px; text-align:center; font-size:11px; color:#aaa; line-height:18px; }
  .receipt-num { font-family:monospace; font-size:12px; color:#888; }
</style>
</head>
<body>
<div class="logo">Q<span>TRIVIA</span> PERÚ</div>
<div class="subtitle">Boleta electrónica — Entrada VIP</div>
<hr class="thick"/>

<div class="row"><span class="label">N° Comprobante</span><span class="receipt-num">VIP-${shortId}</span></div>
<div class="row"><span class="label">Fecha de compra</span><span class="value" style="font-weight:400;font-size:12px;">${date}</span></div>
<div class="row"><span class="label">Estado</span><span class="badge">🎮 Entrada confirmada</span></div>

<hr class="thin"/>
<h3>Detalle</h3>
<div class="big-emoji">🎮</div>
<div class="game-title">${o.game.title}</div>
<br/>
<div class="row"><span class="label">Fecha del juego</span><span class="value">${gameDate}</span></div>
<div class="row"><span class="label">Método de pago</span><span class="value">EXTERNO</span></div>

<hr class="thin"/>
<div class="total-row">
  <span class="total-label">Entrada pagada</span>
  <span class="total-amount">S/ ${o.game.entryFee.toFixed(2)}</span>
</div>

<div class="footer">
  QTrivia Perú · Lima, Perú<br/>
  ${formatDateShort(o.joinedAt)}<br/>
  <em>Este documento es un comprobante de su entrada al juego VIP.</em>
</div>
</body>
</html>`;
}
...
export const OrderDetailScreen: React.FC<Props> = ({ navigation, route }) => {
  const { type, order } = route.params as { type: 'merch' | 'life' | 'vip'; order: MerchGroup | LifeOrder | VipEntry };
  const [downloading, setDownloading] = useState(false);

  const isMerch = type === 'merch';
  const isVip = type === 'vip';
  const mg = order as MerchGroup;
  const lo = order as LifeOrder;
  const ve = order as VipEntry;

  const handleDownloadReceipt = async () => {
    setDownloading(true);
    try {
      const html = isMerch ? buildMerchReceiptHtml(mg) : isVip ? buildVipReceiptHtml(ve) : buildLifeReceiptHtml(lo);
  ...
  const statusCfg = isMerch ? (STATUS_CONFIG[mg.status] ?? STATUS_CONFIG.PENDING) : null;
  const orderLabel = isMerch ? fmtOrderNum(mg.orderNumber) : isVip ? fmtOrderNum(ve.orderNumber) : fmtOrderNum(lo.orderNumber);
  const orderDate = isVip ? ve.joinedAt : isMerch ? mg.createdAt : lo.createdAt;
  ...
  return (
    ...
    >
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        {/* Hero: ID + status */}
        <View style={[styles.heroCard, !isMerch && !isVip && styles.heroCardLife, isVip && styles.heroCardVip]}>
          <Text style={styles.heroEmojis}>
            {isMerch ? mg.items.slice(0, 4).map(i => i.emoji).join(' ') : isVip ? '🎮' : '❤️'}
          </Text>
          <Text style={styles.heroId}>{orderLabel}</Text>
          <Text style={styles.heroDate}>{formatDate(orderDate)}</Text>
          {statusCfg && (
          ...
          {!isMerch && !isVip && (
            <View style={[styles.statusBadge, { backgroundColor: '#EC489922', borderColor: '#EC489955' }]}>
              <Text style={[styles.statusText, { color: '#EC4899' }]}>✅ Acreditado</Text>
            </View>
          )}
          {isVip && (
            <View style={[styles.statusBadge, { backgroundColor: '#FACC1522', borderColor: '#FACC1555' }]}>
              <Text style={[styles.statusText, { color: '#FACC15' }]}>🎮 Entrada confirmada</Text>
            </View>
          )}
        </View>

        {/* Products / Pack / VIP detail */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>
            {isMerch ? `PRODUCTOS (${mg.items.length})` : isVip ? 'DETALLE DEL JUEGO' : 'DETALLE'}
          </Text>

          {isMerch ? (
            <>
              {mg.items.map((item, idx) => (
                <View key={item.orderId} style={[styles.itemRow, idx < mg.items.length - 1 && styles.itemRowBorder]}>
              ...
              <View style={styles.totalRow}>
                ...
                <Text style={styles.totalAmount}>S/{mg.totalAmount.toFixed(2)}</Text>
              </View>
            </>
          ) : isVip ? (
            <View style={styles.detailGrid}>
              <Row label="Juego" value={ve.game.title} fullWidth />
              <Row label="Fecha del juego" value={formatDate(ve.game.scheduledAt)} fullWidth />
              <Row label="Entrada pagada" value={`S/${ve.game.entryFee.toFixed(2)}`} highlight />
            </View>
          ) : (
            <View style={styles.detailGrid}>
              <Row label="Pack" value={lo.packLabel} />
              <Row label="Vidas por pack" value={`${lo.lives} ❤️`} />
              <Row label="Cantidad" value={`${lo.quantity} pack${lo.quantity !== 1 ? 's' : ''}`} />
              <Row label="Total vidas" value={`${lo.lives * lo.quantity} ❤️`} />
              <Row label="Total pagado" value={`S/${lo.price.toFixed(2)}`} highlight />
            </View>
          )}
        </View>
        ...
        {!isVip && (
          <View style={styles.section}>
            ...
            <View style={styles.detailGrid}>
              <Row label="Método" value={(lo.method ?? mg.method ?? '').toUpperCase()} />
              {isMerch && <Row label="Total" value={`S/${mg.totalAmount.toFixed(2)}`} highlight />}
            </View>
          </View>
        )}

        {/* Shipping info */}
        {isMerch && (mg.recipientName || mg.address || mg.phone || mg.dni) && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>DATOS DEL DESTINATARIO</Text>
            <View style={styles.detailGrid}>
              {mg.recipientName && <Row label="Nombre" value={mg.recipientName} />}
              {mg.dni && <Row label="DNI" value={mg.dni} />}
              {mg.phone && <Row label="Teléfono" value={mg.phone} />}
              {mg.address && <Row label="Dirección" value={mg.address} fullWidth />}
              {mg.notes && <Row label="Notas" value={mg.notes} fullWidth />}
            </View>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  heroCardLife: { backgroundColor: 'rgba(236,72,153,0.12)', borderColor: 'rgba(236,72,153,0.25)' },
  heroCardVip: { backgroundColor: 'rgba(250,204,21,0.1)', borderColor: 'rgba(250,204,21,0.3)' },
